...
if sauvez_femme == 'oui':
    ...
    if sauvez_femme == 'oui':
        ...
        print(
            "vous la laisser mourir, ce n'est pas bien vortre metier c'est de sauver les gens pas de les laisser mourire. Mais bon")
        sleep(2)
        print(
            "Une heure après votre chef vous convoque dans son bureau. Il vous demande comment c'est passer votre première intervention.")
        mentir = input("voulez vous lui mentir? ")
        if mentir == 'oui':
            sleep(2)
            if aléatoire == 1:
